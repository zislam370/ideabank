<form autocomplete="off" method="post" action="{{ route('form_stack_holders',$form_stack_holder->activity_id) }}"
      class="form-horizontal margin-none" novalidate="novalidate">
    <!-- CSRF Token -->
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />

    {{ Form::hidden( 'idea_id', Input::old('idea_id',$form_stack_holder->idea_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'step_id', Input::old('step_id',$form_stack_holder->step_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'activity_id', Input::old('step_id',$form_stack_holder->activity_id), array('class'=>'form-control')) }}

    <table id="tbl-stack_holder" class="table table-bordered table-white">
        <tbody>
        <?php $i = 0;?>
        @foreach ($form_stack_holder->items as $item)
        <tr id="{{$i}}">
            <td class="col-sm-1">
            ...
            <td>
                <table  class="table table-bordered table-condensed">
                    <tr>
                        ...
                        <td>
                            <input type="text" name="item[{{{$i}}}][role]" placeholder="{{Lang::get('form/title.role')}}"  value="{{{$item->role}}}" class="role form-control">
                        </td>
                    </tr><tr>
