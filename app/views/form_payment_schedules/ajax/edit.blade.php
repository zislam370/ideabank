<form autocomplete="off" method="post" action="{{ route('form_payment_schedules',$form_payment_schedule->activity_id) }}"
      class="form-horizontal margin-none" novalidate="novalidate">
    <!-- CSRF Token -->
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />

    {{ Form::hidden( 'idea_id', Input::old('idea_id',$form_payment_schedule->idea_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'step_id', Input::old('step_id',$form_payment_schedule->step_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'activity_id', Input::old('step_id',$form_payment_schedule->activity_id), array('class'=>'form-control')) }}

    <table id="tbl-payment_schedule" class="table table-bordered table-white">
        <tbody>
        <?php $i = 0;?>
        @foreach ($form_payment_schedule->items as $item)
        <tr id="{{$i}}">
            <td class="col-sm-1">
                <a class="btn btn-danger" href="javascript:;" onclick="remove_payment_schedule_row(this)">-</a>
            </td>
            <td>
    ...
    <a class="btn btn-info" href="javascript:;" onclick="add_payment_schedule_row()">+</a>
...
<script>
    function add_payment_schedule_row(){
        var $cln = $("#tbl-payment_schedule > tbody > tr:last");
        ...
        $("#tbl-payment_schedule").append($new);

        auto_init_components();
    }
    function remove_payment_schedule_row(t){
        var cnt = $("#tbl-payment_schedule > tbody > tr").length;
        if(cnt>1){
            var $tr = $(t).closest('tr');
            $tr.remove();
        }
        $('#tbl-payment_schedule tr:first-child a.btn-danger').hide();
    }
