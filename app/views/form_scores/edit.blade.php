...
<form autocomplete="off" method="post" class="form-horizontal margin-none" novalidate="novalidate">
    ...
    <div class="widget">
        ...
        <div class="widget-head">
            <h4 class="heading">@lang('form_scores/title.create.sub')</h4>
        </div>
        <!-- // Widget heading END -->
        ...
        <div class="widget-body innerAll inner-2x">
            ...
            <table class="table table-bordered table-striped table-white">
                <?php $i = 0;?>
                @foreach ($form_score as $score)
                <tr>
                    <td>
                        {{{$score->heads->name}}} <input type="hidden" name="budget[{{{$i}}}][head_id]" value="{{{$score->heads->id}}}">
                    </td>
                    <td>
                        <input type="text" name="score[{{{$i}}}][score]" placeholder="Score"  value="{{{$score->amount}}}" class="form-control">
                    </td>
                    <td>
                        <input type="text" name="budget[{{{$i}}}][comment]" placeholder="Comment" value="{{{$score->comment}}}" class="form-control">
                    </td>
                </tr>
                <?php $i++ ?>
                @endforeach
            ...
            <div class="form-actions">
                <a class="btn btn-link" href="{{ route('form_scores') }}">@lang('button.cancel')</a>
