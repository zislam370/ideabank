    ...
    <div class="widget">
        ...
        <div class="widget-body innerAll inner-2x">
            ...
            <table class="table table-bordered table-striped table-white">
                <?php $i = 0;?>
                @foreach ($form_fund as $fund)
                <tr>
                    <td>
                        {{{$fund->heads->name}}} <input type="hidden" name="fund[{{{$i}}}][head_id]" value="{{{$fund->heads->id}}}">
                    </td>
                    <td>
                        <input type="text" name="fund[{{{$i}}}][amount]" placeholder="Amount"  value="{{{$fund->amount}}}" class="form-control">
                    </td>
                    <td>
                        <input type="text" name="fund[{{{$i}}}][comment]" placeholder="Comment" value="{{{$fund->comment}}}" class="form-control">
                    </td>
                </tr>
                <?php $i++ ?>
                @endforeach
