<form autocomplete="off" method="post" action="{{ route('form_sp_panels',$form_sp_panel->activity_id) }}"
      class="form-horizontal margin-none" novalidate="novalidate">
    <!-- CSRF Token -->
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />

    {{ Form::hidden( 'idea_id', Input::old('idea_id',$form_sp_panel->idea_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'step_id', Input::old('step_id',$form_sp_panel->step_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'activity_id', Input::old('step_id',$form_sp_panel->activity_id), array('class'=>'form-control')) }}

    <table id="tbl-sp_panel" class="table table-bordered table-white">
        <tbody>
        <?php $i = 0;?>
        @foreach ($form_sp_panel->items as $item)
        <tr id="{{$i}}">
            <td class="col-sm-1">
                <a class="btn btn-danger" href="javascript:;" onclick="remove_sp_panel_row(this)">-</a>
            </td>
            <td>
    ...
    <a class="btn btn-info" href="javascript:;" onclick="add_sp_panel_row()">+</a>
...
<script>
    function add_sp_panel_row(){
        var $cln = $("#tbl-sp_panel > tbody > tr:last");
        ...
        $("#tbl-sp_panel").append($new);
        auto_init_components();
    }
    function remove_sp_panel_row(t){
        var cnt = $("#tbl-sp_panel > tbody > tr").length;
        if(cnt>1){
            var $tr = $(t).closest('tr');
            $tr.remove();
        }
        $('#tbl-sp_panel tr:first-child a.btn-danger').hide();
    }
