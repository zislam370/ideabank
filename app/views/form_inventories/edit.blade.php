    ...
    <div class="widget">
        ...
        <div class="widget-body innerAll inner-2x">
            ...
            <table class="table table-bordered table-striped table-white">
                <?php $i = 0;?>
                @foreach ($form_budget as $budget)
                <tr>
                    <td>
                        {{{$budget->heads->name}}} <input type="hidden" name="budget[{{{$i}}}][head_id]" value="{{{$budget->heads->id}}}">
                    </td>
                    <td>
                        <input type="text" name="budget[{{{$i}}}][amount]" placeholder="Amount"  value="{{{$budget->amount}}}" class="form-control">
                    </td>
                    <td>
                        <input type="text" name="budget[{{{$i}}}][comment]" placeholder="Comment" value="{{{$budget->comment}}}" class="form-control">
                    </td>
                </tr>
                <?php $i++ ?>
                @endforeach
