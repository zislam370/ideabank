<form autocomplete="off" method="post" action="{{ route('form_deliverables',$form_deliverable->activity_id) }}"
      class="form-horizontal margin-none" novalidate="novalidate">
    <!-- CSRF Token -->
    <input type="hidden" name="_token" value="{{ csrf_token() }}" />

    {{ Form::hidden( 'idea_id', Input::old('idea_id',$form_deliverable->idea_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'step_id', Input::old('step_id',$form_deliverable->step_id), array('class'=>'form-control')) }}
    {{ Form::hidden( 'activity_id', Input::old('step_id',$form_deliverable->activity_id), array('class'=>'form-control')) }}

    <table id="tbl-deliverable" class="table table-bordered table-white">
        <tbody>
        <?php $i = 0;?>
        @foreach ($form_deliverable->items as $item)
        <tr id="{{$i}}">
            <td class="col-sm-1">
    ...
    <a class="btn btn-info" href="javascript:;" onclick="add_deliverable_row()">@lang('button.add')</a>
