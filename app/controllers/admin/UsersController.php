...
class UsersController extends AdminController
{
    ...
    protected $validationRules = array(
        'first_name'       => 'required|alpha_space|between:3,255',
...
//        }
//
//        var_dump($users); return;
        // Paginate the users
        if (Input::get('first_name')) {
            $users = $users->where('first_name','LIKE', '%'.e(Input::get('first_name')).'%');
        }
        if (Input::get('role')) {
            $users = $users
                ->join('users_groups', 'users_groups.user_id', '=', 'users.id')
                ->where('users_groups.group_id','=', e(Input::get('role')));
        }
        if (Input::get('user_type')) {
            $users = $users->where('user_type','=', e(Input::get('user_type')));
        }
        ...
        // Show the page
        return View::make('backend/users/index', compact('users','roles'))->with('input', Input::all());
    }
    ...
    public function postCreate()
    {
        ...
        $this->validationRules['email'] = "email|unique:users,email,".Input::get('email').",email";
        $this->validationRules['mobile'] = "required|mobile|unique:users|size:11";
...
//        $this->validationRules['mobile'] = "required|unique:users,mobile,{$user->mobile},mobile";
