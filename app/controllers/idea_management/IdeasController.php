...
class IdeasController extends AuthorizedController {
    ...
    {
        if(!$this->check_access(['sys_config'])){
            return View::make('error/403')->with('warning', 'You do not have access to do this action.');
        }
...
//        var_dump($ideas[0]->category->name);return;
	...
	{
        if(!$this->check_access(['idea.selected_list'])){
            return View::make('error/403')->with('warning', 'You do not have access to do this action.');
        }
...
//        echo $workflow_ids; return;
        if (Input::get('advertisement_id')) {
            $ad_id = explode("-",e(Input::get('advertisement_id')));
            if(sizeof($ad_id)>1){
//                $ideas = $ideas->where('ideas.workflow_category_id','=',$ad_id[0])
                $ideas = $ideas->whereRaw(" FIND_IN_SET(ideas.workflow_category_id,'$workflow_ids') ")
                                ->where('ideas.advertisement_id','=',$ad_id[1]);
            }else{
//                $ideas = $ideas->where('ideas.workflow_category_id','=',$ad_id[0]);
                $ideas = $ideas->whereRaw(" FIND_IN_SET(ideas.workflow_category_id,'$workflow_ids') ");
...
//                $ideas = $ideas->where('ideas.workflow_category_id','=',$ad_id[0]);
        ...
        $workflow_categories = DB::select( DB::raw("SELECT id, w.`name`  FROM `workflow_categories` AS w
                                                WHERE w.`is_periodical` = 0
                                                AND FIND_IN_SET(w.id,'$workflow_ids')
                                                UNION
                                                SELECT CONCAT(a.`workflow_category_id`,'-',a.id) as 'id', CONCAT(w.`name`,' (', a.name,')') FROM `workflow_categories` AS w
                                                INNER JOIN `advertisements` AS a ON a.`workflow_category_id` = w.`id`
                                                WHERE w.`is_periodical` = 1 AND
                                                FIND_IN_SET(w.id,'$workflow_ids')
                                                "
        ));
        $wfs = array();
        foreach($workflow_categories as  $wc){
...
//        return View::make('ideas.unsortedlist', compact('ideas'));
	...
	{
        if(!$this->check_access(['idea.exited_list'])){
            return View::make('error/403')->with('warning', 'You do not have access to do this action.');
        }
...
//        $workflow_categories = Workflow_category::lists('name', 'id');
        if(!$this->check_access(['idea.sorting'])){
            return View::make('error/403')->with('warning', 'You do not have access to do this action.');
        }
